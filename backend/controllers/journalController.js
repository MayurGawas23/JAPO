...
module.exports.submitJournal = async (req, res) => {
  try {
    ...
    if (!journalName || !publisher || !abstract || !description) {
...
// Helper function to add a comma after "Analytical"
function addCommaToTitle(title) {
  let updatedTitle = title.replace(/\b(analytical)\s+(experimental)\b/i, '$1, $2');

  // Remove any abbreviation in parentheses at the end of the title
  updatedTitle = updatedTitle.replace(/\s*\([^)]*\)$/, '').trim();
...
async function saveToDatabase(metadata) {
  ...
  const { 'container-title': journalName, ISSN, volume, issue, title, DOI } = metadata;
  ...
  const journalNameString = Array.isArray(journalName) ? journalName[0] : journalName || "Unknown Journal";
// Add comma after "Analytical" in the journal title
const updatedTitle = addCommaToTitle(journalNameString.trim());
console.log("Updated journal title:", updatedTitle);


  let journal = await JournalModel.findOne({ journalName: updatedTitle });

  if (!journal) {
    journal = new JournalModel({

      journalName : updatedTitle,
...
exports.getAllJournals = async (req, res) => {
  try {
    const journals = await JournalModel.find({}, 'journalName issn publisher short_title coverImg start_year abstract   ');
...
exports.getJournalByISSN = async (req, res) => {
  try {
    ...
    const journal = await JournalModel.findOne(
      { short_title },
      'journalName issn publisher volumes abstract short_title coverImg frequency start_year online_issn subject language format doi country website aimAndScope editorialBoard editorInChief'
    );
...
exports.getVolumesByISSN = async (req, res) => {
  try {
    const { short_title } = req.params;
    const journal = await JournalModel.findOne({ short_title }, 'journalName issn coverImg volumes ');
    if (!journal) return res.status(404).json({ error: 'Journal not found' });
    res.json(journal);
  } catch (error) {
...
exports.getIssuesByVolume = async (req, res) => {
  try {
    ...
    const journal = await JournalModel.findOne({ short_title }, 'volumes  journalName coverImg ');
    if (!journal) return res.status(404).json({ error: 'Journal not found' });

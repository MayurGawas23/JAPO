...
// Assigned Reviewers (with per-reviewer status)
reviewers: [
  {
    reviewer: { type: String, ref: "User", required: true },
...
paperSchema.methods.getAuthorView = function() {
  return {
    journal: this.journalName,
    paperId: this.paperId, // Add journal name for author view
    ...
    status: this.status,
     // Always show under_review to authors
  };
};

...
router.get(
  ...
  async (req, res) => {
    try {
      const reviewer = req.user.username; // must match assign logic

      const assignedPapers = await PaperModel.find({
        reviewers: {
          $elemMatch: {
            reviewer: reviewer,
            submitted: false,
      ...
      // NOTE: empty array is NOT an error
      return res.status(200).json(assignedPapers);
    } catch (err) {
...
router.get('/:paperId', async (req, res) => {
    ...
    try {
      ...
      .findOne({paperId}) // Example for MongoDB
      .populate("suggestedreviewers", "username email affiliation");
      if (!paper) {

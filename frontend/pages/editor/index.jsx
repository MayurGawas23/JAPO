...
const EditorDashboard = ({ papers, token }) => {
  ...
  //     router.push(`/editor?articleId=${selectedArticle._id}`);
  //   };

  const handleReview = (paper) => {
    setSelectedPaper(paper);
    router.push(`/editor/fromreviewer?paperId=${paper._id}`);
  };

  const links = [
    { href: 'editor/submitjournal', label: 'Publish A Journal', icon: <BookA /> },
    { href: 'editor/edjournals', label: 'Manage Journals', icon: <FileText /> },
    { href: '/editor/assign-reviewers', label: 'Assign Reviewers', icon: <ClipboardCheck /> },
    { href: 'editor/reviews', label: 'Reviews from Reviewer', icon: <Globe /> },
    { href: '', label: 'Manage Reviewers', icon: <Users /> },
    { href: '', label: 'Manage Published Papers', icon: <Archive /> },
    { href: '/editor/editorialboardmembers', label: 'Editorial Board Members', icon: <Users2 /> },
  ];

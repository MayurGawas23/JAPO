  ...
  return (
    <div className="w-full ">
      ...
      <Link
        ...
        <div className="w-full h-full mx-auto bg-bldue-50">
        <div onClick={()=> router.push(`/journals/${journal.short_title}`)} className="h-[200px] bg-reed-300 w-full px-[350px] flex items-center gap-10 p-4 cursor-pointer ">
            <div className="h-[170px] w-[120px] p-1 bg-gray-200">
            <img src={journal.coverImg} className="w-full h-full"></img>
            </div>
          <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold">{journal.journalName}</h1>
          <h1 className="font-bold text-zinc-500">ISSN: {journal.issn}</h1>
          </div>
          </div>
          <div className="h-[40px] w-full  flex items-center px-[320px] mb-4   ">
          ...
          <div className="w-[80%] mx-auto border-t-2 p-4 px-[100px]">
            ...
            <p className="leading-[30px] mb-4"><strong>{journal.journalName} ({journal.short_title})</strong> {journal.abstract}</p>
     ...
     <h1 className="font-semibold text-lg mb-2">Journal information</h1>
            <h1 className=" p-2 ">Title: {journal.journalName}</h1>
            <h1 className="p-2 border-b bg-zinc-200">Short Title : {journal.short_title}</h1>
            <h1 className=" p-2 ">ISSN: {journal.issn}</h1>
            <h1 className="p-2 border-b bg-zinc-200">Frequency: {journal.frequency} </h1>
            <h1 className=" p-2 ">Publisher: {journal.publisher} , {journal.country}</h1>
            <h1 className="p-2 border-b bg-zinc-200">   Editor in Chief: {journal.editorialBoard?.find(member => member.role === "Editor-in-Chief")?.name || "N/A"} </h1>
            <h1 className=" p-2 ">Starting year: {journal.start_year} </h1>
            <h1 className="p-2 border-b bg-zinc-200">Subject : {journal.subject}</h1>
            <h1 className=" p-2 ">Language: {journal.language} </h1>
            <h1 className="p-2 border-b bg-zinc-200">Publication Format: {journal.format}</h1>

            <h1 className=" p-2  ">Email Id: -- {journal.email === "" ? "----" : journal.email}</h1>
            <h1 className="p-2 border-b bg-zinc-200">  Website : {journal.website === "" ? "----" : journal.website}</h1>
     </div>

...
const Index = (props) => {
  ...
  return (
    ...
    <div className="flex-grow w-fullc min-h-screen justify-between  flex flex-col items-center">
      ...
      <div  className="grid lg:grid-cols-1 sm:grid-cols-1 gap-8 w-[70%] pt-10 ">
        <h3 className="font-[poppins-semibold] text-3xl text-center">Our Journal</h3>
        {props.journals.map((journal) => ( // Limit displayed journals to 6 for better fit
          <div
            key={journal.short_title}
            className="bg-zih nc-200 p-4 border mb-4 bg-white rounded shadow-md flex  justify-start items-center cursor-pointer"
          ...
          >
            ...
            <div className=" bg-ziwnc-300  w-full h-full p-4">
              <h2 className="text-2xl font-semibold mb-2 ">{journal.journalName}</h2>
              {journal.issn && journal.issn !== "null" && journal.issn.trim() && (
                <p className="text-lg text-gray-600 font-black mb-4">ISSN: {journal.issn}</p>
              )}
              <p className="line-clamp-2 mb-2 mr-[120px]"><span className="font-bold uppercase">{journal.short_title}</span> {journal.abstract}</p>

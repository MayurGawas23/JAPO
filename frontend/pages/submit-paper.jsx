...
const SubmitPaper = ({ reviewers }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log("Submitting with Journal Name:", journalName); // Debug log
      ...
      if (suggestedReviewers.length !== 2) {
        setMessage("Please select exactly 2 reviewers");
        return;
      }
      ...
      console.error("Submission Error:", err.response?.data || err.message); // Log the full error
      ...
      <div className="flex flex-col justify-center items-center p-10 bg-gray-100">
          ...
          <form className="bg-white p-6 shadow-md rounded w-full" onSubmit={handleSubmit}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setJournalName(selectedJournal ? selectedJournal.journalName : ""); // Set the journal name
              }}
            >
              <option value="">Select Journal</option>
              {journals.map((journal) => (
                <option key={journal.short_title} value={journal.short_title}>
                  {journal.journalName} ({journal.short_title})
                </option>
              ))}
            </select>
            ...
            {/* Index Terms */}
            <input type="text" className="w-full border rounded-lg p-2 " placeholder="Index Terms(comma seperated)" value={indexTerms} onChange={(e) => setIndexTerms(e.target.value)} required />
            {/* Co-Auhtors */}
        ...
        `}
            ...
            <p className="text-sm text-gray-500 mt-2">
              Selected {suggestedReviewers.length} / 2 reviewers
            </p>

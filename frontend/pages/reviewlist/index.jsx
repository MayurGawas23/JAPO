...
const SubmitReview = ({ papers, initialMessage, token }) => {
  ...
  const handleReview = (paper) => {
    setSelectedPaper(paper);
    router.push(`/reviewlist/submitreview?paperId=${paper.paperId}`);
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      <Header />
      <div className="p-6 flex-1  h-full w-[50%]">
        ...
        <ul className="border rounded p-2 w-full">
          {Array.isArray(papers) && papers.length > 0 ? (
            papers.map((paper) => (
              <li
                key={paper.paperId}
                className={`cursor-pointer p-2 border-b last:border-none flex justify-between items-center px-14 ${selectedPaper?.paperId === paper.paperId ? "bg-blue-100" : "hover:bg-gray-100"
                  }`}
              >
                <div className="flex">
                  <File />

...
import Logo from './PubLogo';
import MainLogo from './MainLogo'
import PubLogo from './PubLogo'
...
const Header = ( {openAuthDialog,  setOpenAuthDialog}) => {
...
const getLogoComponent = () => {
  if (router.pathname.startsWith('/journals')) return <MainLogo />;
  // if (router.pathname.startsWith('/books')) return <BookLogo />;
  // if (router.pathname.startsWith('/conferences')) return <ConLogo />;
  return <MainLogo />;
};


  return (
    <header className=" sm:w-full md:w-full  lg:w-full   border-b  mx-auto sm:px-1 md:px-10  lg:px-60 bg-grady-200 text-black pt-1 border-gray-200">
      <div className="flex justify-between items-center h-[80px]">
        {/* Logo Section */}
        <Link href="/" aria-label="JAPO Publishers Home">
        ...
        <div className="flex items-center gap-4 ml-auto">
          ...
          <div className="hidden lg:flex items-center gap-4">
            <Link href="/about" className="hover:underline">About JAPO</Link>
            <Link href="/policy-ethics" className="hover:underline">Publication Policy & Ethics</Link>
              ...
              <div className="flex items-center gap-3">
                {router.pathname.startsWith("/journals") &&  (
                  <div className="flex items-center gap-3">
                    ...
                    {user.role === "editor" && (
                      <Link href="/editor/assign-reviewers" className="mx-2 bg-[#ffa73b] p-1 rounded-sm px-2 text-white font-medium">
                        Assign Reviewers
                      </Link>
                    )}
                    {user.role === "reviewer" && (
                      <Link href="/reviewlist" className="mx-2 bg-[#ffa73b] p-1 rounded-sm px-2 text-white font-medium">
                        Submit Review
                      </Link>
                    )}
                  </div>
                ...
                <DropdownMenuContent className="w-[250px] p-2">
                  ...
                  >
                    ...
                    {user.role === "editor" && (
                        ...
                        <p  onClick={() => router.push('/editor')}>Editor DashBoard</p>
                    )}
                     {user.role === "reviewer" && (
